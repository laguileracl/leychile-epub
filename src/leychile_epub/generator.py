...
from .styles import get_premium_css, CHILEAN_BLUE, CHILEAN_RED, ACCENT_GOLD
...
'''
    ...
    def _create_keyword_index_page(self) -> None:
        ...
        for keyword, refs in sorted_keywords:
            ...
            refs_html = ", ".join(
                [f'<a href="{r["ref"]}">Art. {r["art"]}</a>' for r in refs[:8]]
            )
            sections_html.append(
        ...
        for chapter, content in self.toc_sections:
            ...
            for item in content:
                if item.get("type") == "parrafo":
                    parrafo_text = item.get("text", "")
                    if parrafo_text:
                        short_text = parrafo_text[:40] + "..." if len(parrafo_text) > 40 else parrafo_text
                        sub_items.append(
                            epub.Link(
                                chapter.file_name,
                                short_text,
